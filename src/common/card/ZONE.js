...
export default function DefaultCard({
  ...
  const code = _.get(nodeTypeObj, 'code', '').split('-')[1] || 'Cảm biến';
  ...
  const nodePower = _.get(JSON.parse(state), 'power', 1) === 1 ? true : false;
  ...
  return (
    <Card onClick={handleClick} className={cx(styles['card'], stylesStatus)}>
      ...
      <CardContent className={cx([styles['card__front'], stylesStatus])}>
        ...
        <Box
          mt="20px"
          className={cx([styles['card__content'], styles['center']])}
        >
          {nodePower ? (
